...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), net_args=dict(), n_sample=80, 
    ...
    def compute_loss(count, logphis, thetas, logphis0, thetas0,
        ...
        if output_mode == 'phi':
            phiold_phinew = (count*torch.exp(delta_logphi)).sum()
            phinew_phiold = phiold_phinew.conj()
            phiold_phiold = count.sum()
            phinew_phinew = (count*torch.exp(2*delta_logphi)).sum()
            
            dfs = (phiold_phinew*phinew_phiold/phiold_phiold/phinew_phinew).real
            
            ws = count*torch.exp(2*delta_logphi)
            ws = ws/phinew_phinew
            ...
            # loss_re = (loss_re + 0.01*weights*delta_logphi**2).sum()
            loss_re = loss_re.sum() + (gamma*weights*delta_logphi**2).sum() #- gamma*dfs
        ...
        return loss_re, 1 - dfs.item(), (weights*(1-torch.cos(delta_theta))).sum().item(), me_real, cme_real
        #return loss_re, (weights*(1-torch.cos(delta_theta))).sum().item(), me_real, cme_real

    # # setting optimizer
    optimizer_theta = torch.optim.Adam(psi_model.model_theta.parameters(), lr=2e-4)
    ...
    def update_theta(n_optimize, preload_size, batch_size, sd):
        ...
        for i in range(n_optimize):
            ...
            loss_e, infid, angtol, me, cme_real = compute_loss(counts, logphis, thetas, logphi0, theta0,
            ...
            if verbose and i%(n_optimize//5) == 0:
                print('me: {:.4f}, dfs: {:.4f}, logmincut: {:.4f}, logmaxcut: {:.4f}, infid: {:.4f}, beta: {:.2f}'.format(me.item()/TolSite, dfs.item(), np.log(mincut), np.log(maxcut), infid, beta))
        ...
        return dfs, angtol, infid, me, cme_real, er, clipfrac, i
    ...
    def update_logphi(n_optimize, preload_size, batch_size, sd, target):
        ...
        for i in range(n_optimize):
            ...
            delta_logphi_ops = compute_ops_delta(logphis, op_coeffs, op_ii, pre_op_states,
                                sd, preload_size, batch_size, mode='phi')
            loss_e, infid, _, me, cme_real = compute_loss(counts, logphis, thetas, logphi0, theta0,
                                            weights, clip_ws, delta_logphi_ops, delta_theta_ops, op_coeffs, 
            ...
            gamma = np.clip(gamma, min_beta, 2*max_beta)
            ...
            if verbose and i%(n_optimize//5) == 0:
                print('me: {:.4f}, dfs: {:.4f}, logmincut: {:.4f}, logmaxcut: {:.4f}, infid: {:.4f}, gamma: {:.2f}'.format(me.item()/TolSite, dfs.item(), np.log(mincut), np.log(maxcut), infid, gamma))
    ...
    for epoch in range(epochs):
        ...
        if epoch < warm_up_sample_length:
            DFS, AngTol, IFD, ME, CME, ER, clipfrac, idx = update_theta(n_optimize, preload, batch, buffer._sd)

        else:
            DFS, AngTol, IFD, ME, CME, ER, clipfrac, idx = update_theta(n_optimize, preload, batch, buffer._sd)

            if IFD < 0.1 or not phase_constriction:
                # psi_gpu = psi_model(state_gpu) 
                # logphis = psi_gpu[:,0].cpu().detach().numpy()
