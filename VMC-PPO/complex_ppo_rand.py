...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), n_sample=80, init_type='rand', n_optimize=10,
    ...
    def compute_loss(count, logphis, thetas, thetas0, counts, op_coeffs, op_ii, pre_op_states,
        ...
        return loss_re.sum(), (counts[...,None]/counts.sum()*(1-torch.cos(delta_theta))).sum().detach(),\
               me_real, cme_real
    ...
    def update(n_optimize, preload_size, batch_size, sd, target, DFS_fac):
        # off-policy update
        ...
        for i in range(n_optimize):
            ...
            loss_e, angtol, me, cme_real = compute_loss(counts, logphis, thetas, theta0, counts,
            ...
            if i%(n_optimize//5) == 0:
                print(me/TolSite, dfs, angtol, beta)

            if abs(me - cme_old) < 1e-8:
                logger.debug(
