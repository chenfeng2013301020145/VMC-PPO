...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), net_args=dict(), n_sample=80, 
          ...
          seed=0, input_fn=0, load_state0=True, output_fn='test', TolSite=1, max_beta=10.0, verbose=True):
    """
    ...
    def compute_psi(states, count, logphi0, theta0):
        ...
        with torch.no_grad():
            ...
            weights = count*ratio
            clip_ws = count*torch.clamp(ratio, clip_ratio, 1+clip_ratio)
            # norm = weights.sum()
            weights = (weights/weights.sum())
            clip_ws = (clip_ws/clip_ws.sum())
            
            clipped = ratio.gt(1+clip_ratio) | ratio.lt(clip_ratio)
    ...
    def compute_loss(count, logphis, thetas, logphis0, thetas0,
                    weights, clip_ws, ops_real, ops_imag, beta, output_mode='phi'):   
        ...
        return loss_re.sum(), (weights*(1-torch.cos(delta_theta))).sum().detach(), me_real, cme_real
    ...
    scheduler_theta = torch.optim.lr_scheduler.MultiStepLR(optimizer_theta, [warm_up_sample_length+300], gamma=0.5)
    scheduler_phi = torch.optim.lr_scheduler.MultiStepLR(optimizer_phi, [warm_up_sample_length+300], gamma=0.5)
    ...
    def update_theta(n_optimize, preload_size, batch_size, sd):
        ...
        for i in range(n_optimize):
            ...
            loss_e, angtol, me, cme_real = compute_loss(counts, logphis, thetas, logphi0, theta0,
                                            weights, clip_ws, ops_real, ops_imag, beta, output_mode='theta')
            ...
            beta = np.clip(beta, 0.5, max_beta)
    ...
    def update_logphi(n_optimize, preload_size, batch_size, sd, target):
        ...
        for i in range(n_optimize):
            ...
            loss_e, angtol, me, cme_real = compute_loss(counts, logphis, thetas, logphi0, theta0,
                                            weights, clip_ws, ops_real, ops_imag, beta=0, output_mode='phi')
            ...
            if verbose and i%(n_optimize//5) == 0:
                print('me: {:.4f}, dfs: {:.4f}, logmincut: {:.4f}, logmaxcut: {:.4f}, angtol: {:.4f}, beta: {:.2f}'.format(me.item()/TolSite, dfs.item(), np.log(mincut), np.log(maxcut), angtol, 0))

            loss_e.backward()
            optimizer_phi.step()
        
        return dfs, 0, me, cme_real, 0, clipfrac, i
    ...
    for epoch in range(epochs):
        ...
        state_gpu = torch.from_numpy(states).float().to(gpu)
        ...
        if epoch < warm_up_sample_length:
            DFS, AngTol, ME, CME, ER, clipfrac, idx = update_theta(50, preload, batch, buffer._sd)
        ...
        if epoch >= warm_up_sample_length + 300:
            n_optimize = 10
            #learning_rate = 1e-4
