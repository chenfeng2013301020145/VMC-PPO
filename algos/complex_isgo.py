...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), n_sample=80, init_type='rand', n_optimize=10,
          learning_rate=1E-4, state_size=[10, 2], preload_size=2000, batch_size=3000, 
          target_dfs=0.01, save_freq=10, net_args=dict(), threads=4, warmup_length=500, warm_up_sample_length=10,
    ...
    def compute_psi(sym_states, sym_ii, count, logphi0, theta0, only_theta):
        ...
        return logphis, thetas, weights, dfs.abs()
    ...
    def update(n_optimize, preload_size, batch_size, sd, get_eops, only_theta):
        # off-policy update
        ...
        for i in range(n_optimize):
            ...
            loss_e, me =compute_loss(counts, logphis, thetas, op_coeffs, op_ii, pre_op_states,
                                weights, sd, preload_size, batch_size, get_eops)
    ...
    for epoch in range(epochs):
        ...
        #     optimizer.param_groups[1]['lr'] = 10*learning_rate
        #     warmup_n_sample = n_sample // 2
        #     DFS_fac = 50
        ...
        DFS, ME, AvgE, AvgE2, idx = update(set_op_steps, preload, batch, sd, 
                     get_eops=MHsampler.cal_ops, only_theta=psi_model._only_theta)
