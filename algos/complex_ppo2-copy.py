...
from algos.core import mlp_cnn_sym, get_paras_number, gradient
...
def train(epochs=100, Ops_args=dict(), Ham_args=dict(), n_sample=80, init_type='rand', n_optimize=10,
          learning_rate=1E-4, state_size=[10, 2], preload_size=2000, batch_size=2000, clip_ratio=0.2, 
    ...
    logger.info('Num_of_params: {}'.format(get_paras_number(psi_model, psi_model.filter_num)))
    ...
    def compute_psi(sym_states, sym_ii, count, logphi0, theta0, only_theta):
        ...
        return logphis, thetas, weights, clip_ws, dfs.abs(), clipfrac
    ...
    def compute_loss(count, logphis, thetas,
                    weights, clip_ws, ops_real, ops_imag):   
        ...
        cE_re = ops_real*logphis - cme_real*logphis + ops_imag*thetas
        ...
        return loss_re.sum(), me_real, cme_real
    ...
    for name, param in psi_model.named_parameters():
        if 'conv_re' in name or 'linear_re' in name:
            real_part.append(param)
        elif 'conv_im' in name or 'linear_im' in name:
    ...
    def update_theta(n_optimize, preload_size, batch_size, sd, 
                target, only_theta):
        optimizer = torch.optim.Adam([{'params': imag_part},
                                {'params': activation}], lr=learning_rate)
        ...
        for i in range(n_optimize):
            optimizer.zero_grad()
            logphis, thetas, weights, clip_ws, dfs, _ \
                = compute_psi(sym_states, sym_ii, counts, logphi0, theta0, only_theta)

            if dfs > 50*target:
                logger.debug(
                'early stop at step={} as reaching maximal FS distance in updating thetas'.format(i))
                break
            ...
            ops_real, ops_imag = compute_ops_reim(logphis, thetas, op_coeffs, op_ii, pre_op_states,
                                            sd, preload_size, batch_size)
            loss_e, me, cme_real = compute_loss(counts, logphis, thetas,
                                            weights, clip_ws, ops_real, ops_imag)

            max_me = torch.max(me, cme_real)
    ...
    def update_logphi(n_optimize, preload_size, batch_size, sd, 
            target, only_theta):
        # off-policy update
        optimizer = torch.optim.Adam([{'params':real_part}], lr=learning_rate)
        optimizer.param_groups[0]['lr'] = learning_rate
        sym_states, sym_ii, counts, logphi0, theta0, op_coeffs, op_ii, pre_op_states \
            = buffer.get_states()
        ...
        # data = buffer.get(batch_size=batch_size, batch_type='cutoff', get_eops=not(cal_ops))
        cme_old = 0
        for i in range(2*n_optimize):
            optimizer.zero_grad()
            logphis, thetas, weights, clip_ws, dfs, clipfrac \
                = compute_psi(sym_states, sym_ii, counts, logphi0, theta0, only_theta)
        ...
        return dfs, me, cme_real, clipfrac, AvgE, AvgE2, i
    ...
    for epoch in range(epochs):
        ...
        MHsampler._model.load_state_dict(psi_model.state_dict())
        states, sym_states, counts, update_states, update_psis, update_coeffs, efflens\
        ...
        update_theta(n_optimize, preload, batch, sd,
                    target_dfs, only_theta=psi_model._only_theta)
        ...
        DFS, ME, CME, clipfrac, AvgE, AvgE2, idx = update_logphi(n_optimize, preload, batch, sd,
                        target_dfs, only_theta=psi_model._only_theta)
        ...
        logger.info('Epoch: {}, AvgE: {:.5f}, ME: {:.5f}, CME: {:.5f}, StdE: {:.5f}, DFS: {:.5f}, ClipFrac: {:.3f}, StopIter: {}, IntCount: {}, SymIntCount: {}, A: {:.3f}, SampleTime: {:.3f}, OptimTime: {:.3f}, TolTime: {:.3f}'.
            format(epoch, AvgE/TolSite, ME/TolSite, CME/TolSite, StdE, DFS, clipfrac, idx, IntCount, SymIntCount, alpha, sample_toc-sample_tic, op_toc-op_tic, time.time()-tic))
