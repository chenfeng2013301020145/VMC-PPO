...
# -------------------------------------------------------------------------------------------------
# symmetry functions
def get_max_label(full_x, L, W):
    # input shape of full_x: (batch_size, sym_N, Dp, 2*L-1, 2*W-1)
    full_xv = full_x[:,:,0,:,:]
    batch_size, sym_N, pad_L, pad_W = full_xv.shape[0],full_xv.shape[1],full_xv.shape[-2], full_xv.shape[-1]
...
class sym_model(nn.Module):
    ...
    def symmetry(self, x):
        ...
        max_label = get_max_label(full_x, L, W)
    ...
    def forward(self, x):
        ...
        sym_x = sym_x_unique[inverse_indices,:]
        return torch.stack((sym_x[:,0], sym_x[:,1]), dim=1)
...
if __name__ == '__main__':
    ...
    np.random.seed(seed)
    logphi_model, _ = mlp_cnn_sym([4,4,2], 4, [2,2], stride0=[1], stride=[1], complex_nn=True, pbc=False,
                           output_size=2, relu_type='selu', bias=True, momentum=[0,0], sym_funcs=[c6rotation, transpose, translation])
    ...
    tt1 = logphi_model(state0).detach().numpy()
