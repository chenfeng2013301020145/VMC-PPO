...
class RealTanh(nn.Module):
    def __init__(self):
        ...
        super(RealTanh, self).__init__()

    def forward(self, x):
        real = x[:,0]
        imag = x[:,1]
        return torch.stack((real, np.pi*torch.tanh(imag)), dim=1)
...
class CNN_complex_layer(nn.Module):
    def __init__(self,K,F_in,F_out,stride,complex_relu,layer_name='mid',
        ...
        complex_conv = ComplexConv(F_in,F_out,self.K,stride,0, 
                        groups=groups, dimensions=dimensions, bias=bias)
        self.conv = nn.Sequential(*[complex_conv, complex_relu])

    def forward(self, x):
        if self.layer_name == '1st':
            # x = torch.stack((x, torch.zeros_like(x)), dim=1)
            x = torch.stack((x, x.clone()), dim=1)
        if self._pbc:
...
def mlp_cnn(state_size, K, F=[4,3,2], stride0=[1], stride=[1], 
    ...
    lattice_shape = state_size[0] if dim == 1 else [state_size[0], state_size[1]]
    layers = len(F)
    ...
    # input_layer = CNN_real_layer(Kre=K, Kim=lattice_shape, F_in=Dp, F_out=F[0], stride=stride0, layer_name='1st', dimensions=dimensions,
    #                                 complex_relu=complex_relu, pbc=pbc, bias=bias, groups=groups)
    ...
    cnn_layers += [CNN_complex_layer(K=K, F_in=F[i-1], F_out=F[i], stride=stride, dimensions=dimensions,
                        complex_relu=complex_relu, pbc=pbc, bias=bias, groups=groups) for i in range(1,layers)]
    # cnn_layers += [CNN_real_layer(Kre=K, Kim=lattice_shape, F_in=F[i-1], F_out=F[i], stride=stride, dimensions=dimensions,
    #                     complex_relu=complex_relu, pbc=pbc, bias=bias, groups=groups) for i in range(1,layers)]
...
def sort_by_label(full_x):
    old_shape = full_x.shape
    
    if len(full_x.shape[3:]) == 2:
        # input shape of full_x: (batch_size, sym_N, Dp, L, W)
        """
        Future: act the symmetry function on the filters
        """
        L, W = full_x.shape[-2], full_x.shape[-1]
        full_xv = full_x[:,:,0,:,:]
        ...
        _, indices = torch.sort(full_xv.reshape(batch_size, -1), dim=1)
        state_size = full_x.shape[2]*full_x.shape[3]*full_x.shape[4]
    else:
        N = full_x.shape[-1]
        full_xv = full_x[:,:,0,:]
        ...
        state_size = full_x.shape[2]*full_x.shape[3]

    a = full_x.shape[1]*torch.ones(full_x.shape[1], dtype=indices.dtype, device=full_x.device)[None,...]*torch.arange(full_x.shape[0], device=full_x.device)[...,None]
    index_list = indices.reshape(-1) + a.reshape(-1)
    full_x = full_x.reshape(full_x.shape[0]*full_x.shape[1], state_size)
    full_x = full_x[index_list,:]
    return full_x.reshape(old_shape)
...
class sym_model(nn.Module):
    def __init__(self, state_size, K, F=[4,3,2], stride0=[1], stride=[1], relu_type='selu', device="cpu",
                pbc=True, bias=True, momentum=[0,0], sym_funcs=[identity], gcnn=False, 
        ...
        if self.gcnn:
            net_state_size[-1] = net_state_size[-1]*self.sym_N
            self.model = mlp_cnn(state_size=net_state_size, K=K, F=np.array(F)*self.sym_N, 
                    stride0=stride0, stride=stride, relu_type=relu_type, 
                    pbc=pbc, bias=bias, momentum=momentum, groups=self.sym_N, alpha=alpha)
        else:
            self.model = mlp_cnn(state_size=state_size, K=K, F=F, stride0=stride0, stride=stride, relu_type=relu_type, 
    ...
    def forward(self, x, apply_sym=True):
        if apply_sym:
            ...
            if self.gcnn:
                # shape of x (batch_size*sym_N, Dp, L, W)
                x = x.reshape([xshape[0], sym_N] + list(xshape[1:]))
                x = sort_by_label(x)
                x = self.model(x.reshape([xshape[0], sym_N*xshape[1]] + list(xshape[2:])))
                return x/sym_N
            else:
                if self.apply_unique:
                    _, indices, inverse_indices = unique(x[:,0], dim=0)
                    x = self.model(x[indices])[inverse_indices,:]/np.sqrt(sym_N)
                    x += logMa/sym_N
                    x = x.reshape(xshape[0], sym_N, 2).sum(dim=1)
                else:
                    x = self.model(x)/np.sqrt(sym_N)
                    x += logMa/sym_N
                    x = x.reshape(xshape[0], sym_N, 2).sum(dim=1)
...
def mlp_cnn_sym(state_size, K, F=[4,3,2], stride0=[1], stride=[1], relu_type='selu', device="cpu",
                ...
                gcnn=False, apply_unique=False, MPphase=False, MPtype='NN', alpha=0.35):
    model = sym_model(state_size=state_size, K=K, F=F, stride0=stride0, 
                stride=stride, relu_type=relu_type, device=device, pbc=pbc, bias=bias, 
                momentum=momentum, sym_funcs=sym_funcs, gcnn=gcnn, apply_unique=apply_unique, 
...
if __name__ == '__main__':
    ...
    np.random.seed(seed)
    logphi_model = mlp_cnn_sym([6,6,2], 4, [8,6,4], stride0=[1], stride=[1], pbc=True, relu_type='snake', bias=True, momentum=[0,0], sym_funcs=[c4rotation], gcnn=False, MPphase=False, MPtype='NN')
